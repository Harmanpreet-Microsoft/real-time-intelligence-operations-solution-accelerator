...
def generate_assets(
    ...
    for site_index, site in enumerate(sites_list):
        for i in range(num_assets_per_site):
            ...
            asset_id = f"A_{1000 + site_index * num_assets_per_site + i}"
            assets.append(
                Asset(
                    Id=asset_id,
                    ...
                    MaintenanceStatus=random.choice([
                        "Done",
                        "Pending",
                        "Scheduled"
                    ])
...
def generate_products(num: int) -> pd.DataFrame:
    ...
    for i in range(num):
        ...
        unit_cost = round(
            (list_price / 2) + random.uniform(1, (list_price / 4)),
            2
        )
...
def generate_historical_events(
    ...
    """Generate historical event data."""
    if start_date is None:
        start_date = datetime.now(timezone.utc)

    if days_back > 90:
        print(
            f"Warning: days_back ({days_back}) exceeds maximum of 90 "
            f"days. Setting to 90."
        )
        days_back = 90
    ...
    print(
        f"Generating historical events from "
        f"{end_date.strftime('%Y-%m-%d %H:%M')} to "
    ...
    while current_time <= start_date:
        ...
        for index, asset in enumerate(assets_list):
            ...
            anomaly_rate = asset_event_anomaly_rates[
                index % len(asset_event_anomaly_rates)
            ]
            event = asset_type.create_random_event(
                asset_id=asset["Id"],
    ...
    print(
        f"✅ Generated {len(events)} historical events across "
        f"{len(assets_list)} assets ({days_back} days of data)"
    )
...
def generate_sample_data(
    ...
    """Generate all sample data for the simulation."""
    ...
    try:
        ...
        if not override_existing:
            existing_files = [
                name for name, path in csv_files.items()
                if os.path.exists(path)
            ]
            if existing_files:
                raise FileExistsError(
                    f"CSV file(s) already exist: "
                    f"{', '.join(existing_files)}. "
                    "Set override_existing=True to overwrite or remove "
                    "existing files."
                )

        print(
            f"Generating {num_sites} sites, "
            f"{num_assets_per_site} assets per site, "
        ...
        if include_events:
            print(
                f"Generating historical events for "
                f"{event_days_back} days..."
            )
